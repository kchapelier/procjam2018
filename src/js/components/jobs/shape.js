...
var generalShader = `#version 300 es
    ...
    float trianglePick (vec2 cuv) {
      ...
      return smoothstep(0., 0.0005, cone(cuv));
    }
    ...
    float biconvex_lens (vec2 cuv) {
      ...
      return smoothstep(1., 1. - 0.0005, dist);
    }
...
function getProgramHQ (context) {
  if (programHQ === null) {
    programHQ = context.createProgram(generalShader + `
      ...
      vec2 hammersley(uint originalSample, int bits) {
        uint revertSample;
        ...
        revertSample = revertSample >> (32 - bits);

        return vec2(float(revertSample), float(originalSample));
      }

      vec4 hqProcess (in vec2 uv) {
        int bits = 3;
        float num = 8.;
        vec4 t = vec4(0.);
        vec2 v = vec2(0.);

        for (uint i = uint(0); i < uint(num); i++) {
          v = (hammersley(uint(i), bits) - (num - 1.) / 2.) / (num - 1.) / resolution.xy;
 ...
 */
function getProgramLQ (context) {
  if (programLQ === null) {
    programLQ = context.createProgram(generalShader + `
      vec4 process (in vec2 uv) {
        float value = 0.;
...
function shapeJob (context, inputs, outputs, parameters, done) {
  var program = parameters.hq ? getProgramHQ(context) : getProgramLQ(context);
